...
const HourlyCardsList = ({weather}: Props) => {
    ...
    if (hourly?.time && hourly?.time.length && currentTime) {
      // Finding index of currentTime inside time array property of hourly
      currentTimeIndex = hourly.time.indexOf(roundedTime(currentTime));
                ...
                >
                  <div className="flex w-20 flex-col items-center justify-between text-xs">
                    <span className="font-medium">
                        {t(
                            'forecastsCard.hourly_weather.time',
                            {
...
export default HourlyCardsList;