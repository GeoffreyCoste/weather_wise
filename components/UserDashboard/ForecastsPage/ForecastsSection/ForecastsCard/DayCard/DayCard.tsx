...
const DayCard = ({day, weather, index}: Props) => {
  ...
  const windDirection = weather?.daily?.winddirection_10m_dominant[index];

  const rotation = getWindIconRotationClass(getWindDirection(windDirection));

  return (
    <div className="w-full px-4 py-8 mb-4 rounded-lg text-blue-700 bg-white dark:text-white dark:bg-blue-950">
      <div className="w-full h-[750px] lg:h-auto flex flex-wrap">

        <div className="basis-full flex gap-x-2">
          <span className="text-sm capitalize">{t(`forecastsCard.weekly_weather.details.day_card.day.${day}`)}</span>
          <h5 className="text-sm font-extrabold capitalize">
            {date && t('forecastsCard.weekly_weather.details.day_card.time', 
              {
            ...
            <div className="w-full flex flex-col flex-1 lg:flex-row">
                ...
                <div className="basis-40 lg:basis-1/2">
                    <div className="flex flex-col lg:h-full justify-center items-center">
                        ...
                        {weather?.daily?.weathercode[index] && <span className="text-xs md:text-sm text-center">{t(`weathercodes.${weather.daily.weathercode[index]}`)}</span>}
                    </div>
                </div>
                ...
                <div className="basis-40 lg:basis-1/2 flex flex-col justify-center items-center lg:flex-row ">
                    ...
                    <div className="basis-full lg:basis-auto flex flex-col">
                        <div className="w-full flex justify-center items-end">
                            ...
                            <div className="w-16 text-right">
                                <p className="text-blue-700 dark:text-white font-extrabold text-3xl">
                                    {weather?.daily?.temperature_2m_max[index] && temperature === 'celsius' && weather.daily.temperature_2m_max[index].toFixed(0)}

                                    {weather?.daily?.temperature_2m_max[index] && temperature === 'fahrenheit' && celsiusToFahrenheit(weather.daily.temperature_2m_max[index]).toFixed(0)}
                        ...
                        <div className="w-full flex justify-center items-start">
                            ...
                            <div className="w-16 text-right">
                                <p className="text-blue-700 dark:text-white font-extrabold text-3xl">
                                    {weather?.daily?.temperature_2m_min[index] && temperature === 'celsius' && weather.daily.temperature_2m_min[index].toFixed(0)}

                                    {weather?.daily?.temperature_2m_min[index] && temperature === 'fahrenheit' && celsiusToFahrenheit(weather.daily.temperature_2m_min[index]).toFixed(0)}
                    ...
                    <div className="basis-full lg:basis-1/3 flex items-center lg:flex-col gap-x-2 lg:gap-x-0 relative">
                        <span className="absolute left-1/2 -translate-x-1/2 top-2 lg:-top-5 lg:left-3 lg:translate-x-0 text-xs font-semibold text-blue-400 dark:text-sky-400 whitespace-nowrap">{t('forecastsCard.weekly_weather.details.selected_day_card.feelsLike')}</span>
                        <div className="w-full flex justify-center items-center gap-x-1 lg:gap-x-0">
                            ...
                            <div className="lg:w-9 lg:text-right">
                                <p className="text-blue-700 dark:text-white font-bold text-xs lg:pb-1">
                                    ...
                                    {weather?.daily?.apparent_temperature_max[index] && temperature === 'celsius' && weather.daily.apparent_temperature_max[index].toFixed(0)}
                                    {weather?.daily?.apparent_temperature_max[index] && temperature === 'fahrenheit' && celsiusToFahrenheit(weather.daily.apparent_temperature_max[index]).toFixed(0)}
                        ...
                        <div className="w-full flex justify-center items-center gap-x-1 lg:gap-x-0">
                            ...
                            <div className="lg:w-9 lg:text-right">
                                <p className="text-blue-700 dark:text-white font-bold text-xs lg:pt-1">
                                    {weather?.daily?.apparent_temperature_min[index] && temperature === 'celsius' && weather.daily.apparent_temperature_min[index].toFixed(0)}
                                    {weather?.daily?.apparent_temperature_min[index] && temperature === 'fahrenheit' && celsiusToFahrenheit(weather.daily.apparent_temperature_min[index]).toFixed(0)}
                                    &deg;
                                    {temperature === 'celsius' ? 'C' : 'F'}
                                </p>
                            </div>
                ...
                <div className="w-full lg:h-52 flex flex-col items-center p-4 lg:border-2 lg:border-blue-300 lg:dark:border-sky-400 rounded-lg">
                    ...
                    <div className="basis-full flex flex-wrap items-end justify-center">
                        ...
                        <div className="w-full md:w-auto text-center">
                            {weather?.daily?.winddirection_10m_dominant[index] && (
